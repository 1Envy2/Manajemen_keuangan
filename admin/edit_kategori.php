...
// Jika ID tidak valid atau tidak ada, redirect ke dashboard admin
...
if ($stmt_get = $conn->prepare($sql_get)) {
    ...
    if ($stmt_get->execute()) {
        ...
        if ($result_get->num_rows == 1) {
            ...
            $category_name = $category_data['name']; // Set nama kategori untuk ditampilkan di form
        } else {
            // Kategori tidak ditemukan, redirect
        ...
        $error_msg = "Gagal mengambil data kategori. Silakan coba lagi nanti.";
    }
    $stmt_get->close();
}


// --- Proses form saat submit untuk update ---
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    // Validasi nama kategori baru
    if (empty(trim($_POST['category_name']))) {
        $name_err = 'Nama kategori tidak boleh kosong.';
    } else {
        $new_category_name = trim($_POST['category_name']);
        
        // Cek apakah nama kategori baru sudah ada untuk kategori lain (bukan kategori yang sedang diedit)
        $sql_check = "SELECT id FROM categories WHERE name = ? AND id != ?";
        if ($stmt_check = $conn->prepare($sql_check)) {
            $stmt_check->bind_param("si", $param_name, $param_id);
            $param_name = $new_category_name;
            ...
            if ($stmt_check->execute()) {
                ...
                $error_msg = "Oops! Terjadi kesalahan saat cek nama kategori. Silakan coba lagi nanti.";
            }
            $stmt_check->close();
        }
    }
    ...
    if (empty($name_err) && empty($error_msg)) {
        ...
        if ($stmt_update = $conn->prepare($sql_update)) {
            $stmt_update->bind_param("si", $param_name, $param_id);
            $param_name = $new_category_name;
            $param_id = $category_id;
            if ($stmt_update->execute()) {
                $success_msg = 'Kategori berhasil diperbarui!';
                $category_name = $new_category_name; // Update variabel untuk tampilan form
            } else {
                $error_msg = "Gagal memperbarui kategori. Silakan coba lagi.";
            }
            $stmt_update->close();
        }
    }
...
<head>
    ...
    <style>
        /* CSS yang sama dengan dashboard admin */
...
<body>
    ...
    <div class="main-content">
        <div class="header">
            <div class="title">
                <h1>Edit Kategori</h1>
                <div class="subtitle">Perbarui Nama Kategori Transaksi</div>
            </div>
        </div>
                ...
                <div class="card-content">
                    ...
                    <form action="<?= htmlspecialchars($_SERVER['PHP_SELF']) . '?id=' . $category_id; ?>" method="post">
                        ...
                        <div class="d-flex justify-content-between">
                            <button type="submit" class="btn btn-primary" style="background-color: var(--color-dark-blue); border-color: var(--color-dark-blue);">Perbarui Kategori</button>
